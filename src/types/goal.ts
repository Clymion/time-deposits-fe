...
export const goalInputSchema = z.object({
  ...
  monthlyAmount: z.any()
    ...
    .refine((val) => val === undefined || (!isNaN(val) && val > 0), { message: 'Monthly saving must be a positive number.' })
    .optional(),
  ...
  targetMonths: z.any()
    ...
    .transform((val) => (String(val ?? '').trim() === '' ? undefined : Number(val)))
    // undefined、または正の数かチェック
    .refine((val) => val === undefined || (!isNaN(val) && val > 0), { message: 'Target months must be a positive number.' })
    .optional(),
